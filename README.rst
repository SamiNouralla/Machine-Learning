...
Surveys were given to over 200 Engineering students at the University of Waterloo for measuring satisfaction of their respective program. Respondents were asked to rank their satisfaction for different attributes in areas such as quality of teaching, access to resources, quality of coop jobs received, etc..

From there, the data gathered from all the survey's was consolidated in Excel, and cleaned up to eliminate empty responses. The results based on user scores of 1-5 were saved as csv files so they can used for data analysis. Several algorithms were then implemented in Python for training the data, with the objective being to find the attribute that most strongly predicts the value of the target variable (level of satisfaction).
